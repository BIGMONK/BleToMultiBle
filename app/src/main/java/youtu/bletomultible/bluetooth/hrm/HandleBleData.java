package youtu.bletomultible.bluetooth.hrm;
...
public class HandleBleData {
    ...
    private static String TAG="HandleBleData";
    ...
    public static void HandleData(byte[] rawData, IBleOperateCallback bleOperateCallback, IUpdateCallBack updateCallback) {
        ...
        if (decodeData != null) {
            //解密数据
            HandleData_version2(rawData, decodeData, bleOperateCallback, updateCallback);
        }
        //原始数据
//        HandleData_version(rawData, bleOperateCallback, updateCallback);
    ...
    public static void HandleData_version2(byte[] oldData, byte[] rawData, IBleOperateCallback bleOperateCallback, IUpdateCallBack updateCallback) {
        if (rawData != null) {
...
//                packageBuf = new HandleBleData.byteBuf((HandleBleData.byteBuf)null);
            ...
            if (packageBuf.pointer == packageBuf.blePkgNumInPkg) {
                ...
                if (packageBuf.verifyCRC()) {
                    packageBuf.AnalyzeData(oldData, packageBuf.pkgNoHeadCrcBuf, bleOperateCallback, updateCallback);
                }
            }
        }
    ...
    private static class byteBuf {
        ...
        void addData(byte[] data) {
            ...
            System.out.println("max="+max
                    +"   blePkgIndex"+blePkgIndex
                    +"   this.packageBuffer.length"+this.packageBuffer.length
            );
            for (int j = 1; j < max; ++i) {
        ...
        void AnalyzeData(byte[] oldData, byte[] data, IBleOperateCallback callback, IUpdateCallBack updateCallback) {
            int i;
            switch (this.packageCMD) {
                case 1:
                    ...
                    if ((data[0] & 255) == 1) {
                        ...
                        this.HandleEcgData(oldData, var10, callback);
        ...
        void HandleEcgData(byte[] oldData, byte[] data, IBleOperateCallback callback) {
            if (this.ecgDataKind == 1) {
                ...
                if (this.ecgDataKind == 2) {
                    ...
                    for (int h = 0; h < oldData.length; h++) {
                        stringBuilder.append(oldData[h] + "    ");
                    }
                    System.out.println(TAG+"原始数据：" + stringBuilder.toString()+"心率："+hr);
